...
export const NewOrder: React.FC<NewOrderProps> = ({ currentUser }) => {
    ...
    useEffect(() => {
        const fetchRequesters = async () => {
            ...
            try {
                // Importar dinamicamente para evitar ciclo ou usar a função já importada se disponível
                const { getServicesByClient } = await import('../services/storageService');
                const services = await getServicesByClient(selectedClientId);

                // 1. Pegar do histórico
                const historyNames = services
                    .map(s => s.requesterName?.trim())
                    .filter(name => name && name.length > 0);

                // 2. Pegar do cadastro do cliente (se houver)
                const registeredNames = selectedClient?.requesters || [];
    ...
    const handleAddressChange = async (type: 'pickup' | 'delivery', index: number, value: string) => {
        // Atualiza o estado imediatamente para o usuário digitar
        if (type === 'pickup') {
        ...
        // Verifica se é um CEP (8 dígitos numéricos, com ou sem hífen)
        const cleanValue = value.replace(/\D/g, '');
        if (cleanValue.length === 8) {
            try {
                // Import dinâmico ou normal se já importado
                const { fetchAddressByCEP } = await import('../services/addressService');
                const address = await fetchAddressByCEP(cleanValue);

                if (address) {
                    toast.success('Endereço encontrado!');
                    // Atualiza com o endereço retornado
                    if (type === 'pickup') {
                ...
                // Silencioso se der erro, apenas não preenche
                console.error(e);
            }
        }
    };
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="relative">
                        <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">
                            Solicitado Por
                            {loadingRequesters && <span className="ml-2 text-[10px] animate-pulse">Carregando...</span>}
                        </label>
                        <div className="flex gap-2">
                            ...
                            <button
                                ...
                                className="p-2.5 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg border border-blue-200 dark:border-blue-800 hover:bg-blue-200 dark:hover:bg-blue-800/50 transition-colors"
                        ...
                        {/* DROPDOWN MANUAL */}
                        {showRequesterList && (
                            <div className="absolute z-10 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl mt-1 max-h-48 overflow-y-auto animate-fade-in custom-scrollbar">
                                {recentRequesters.length === 0 ? (
                                    <div className="p-3 text-xs text-slate-500 dark:text-slate-400 text-center">Nenhum solicitante encontrado.</div>
                                ) : (
                                    recentRequesters.map((name, idx) => (
                                        <button
                                            key={idx}
                                            ...
                                            className="w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border-b border-slate-100 dark:border-slate-700 last:border-0 flex items-center gap-2"
                                        >
